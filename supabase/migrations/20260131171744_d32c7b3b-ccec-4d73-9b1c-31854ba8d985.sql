-- Add personal shopper fee columns to orders table
ALTER TABLE public.orders 
ADD COLUMN IF NOT EXISTS personal_shopper_fee numeric DEFAULT 30,
ADD COLUMN IF NOT EXISTS is_physical_store boolean DEFAULT true;

-- Update the create_secure_order function to include K30 personal shopper fee
...
DECLARE
  ...
  v_personal_shopper_fee numeric := 30; -- K30 flat fee for physical stores
BEGIN
  ...
  -- Calculate total (subtotal + service fee + zone fee + carrier bags + personal shopper fee)
  v_total := v_subtotal + v_service_fee + v_zone_fee + v_carrier_bags_total + v_personal_shopper_fee;

  -- Calculate driver payout (80% of zone fee + 100% of personal shopper fee)
  v_driver_payout := (v_zone_fee * 0.80) + v_personal_shopper_fee;
  ...
  INSERT INTO orders (
    ...
    v_personal_shopper_fee,
    true, -- Physical stores always have shopper fee
    'awaiting_payment'
  )
